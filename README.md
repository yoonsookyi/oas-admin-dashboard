...
| 옵션 | 사용 가능한 탭 | 추가 작업 |
|---|---|---|
| **Option A** | 시스템 상태 · 콘텐츠 인벤토리 · 소유자 없는 콘텐츠 · 업그레이드 진단 · Runbook | 없음 |
...
| 파일 배치 | index.html → OHS htdocs, metrics_server.py · backup.sh → 설치 경로 |
...
| OHS 재시작 | `opmnctl restartproc` 실행 및 동작 확인 |
...
```bash
# OHS DocumentRoot 경로 확인
grep "DocumentRoot" $ORACLE_INSTANCE/OHS/ohs1/conf/httpd.conf

# index.html 복사
cp index.html <OHS_HTDOCS>/oas-dashboard.html
```
...
# 4. OHS ProxyPass 설정 추가
echo "ProxyPass        /sysmgmt  http://localhost:9091" >> $ORACLE_INSTANCE/OHS/ohs1/conf/httpd.conf
echo "ProxyPassReverse /sysmgmt  http://localhost:9091" >> $ORACLE_INSTANCE/OHS/ohs1/conf/httpd.conf
...
Apply → BI Server 재시작:
```bash
opmnctl restartproc ias-component=coreapplication_obis1
```
...
└── /oas-dashboard.html   → index.html 정적 파일       (신규)
```
...
브라우저
 │
 ├─ GET /oas-dashboard.html ──────────────→ OHS → index.html 반환
 │
...
**사용 순서:**
1. 로그 파일 선택 (기본: `obips` — Presentation Services)
...
| 로그 키 | 파일 | 확인 대상 |
|---|---|---|
| `obips` | `sawlog0.log` | 리포트 렌더링 오류, 세션 문제 |
| `nqserver` | `nqserver.log` | BI Server 시작·종료, 쿼리 실패 |
| `nqquery` | `nqquery.log` | 쿼리 실행 이력, 느린 쿼리 |
| `obisch` | `obisch1.log` | 스케줄 실행 오류 |
| `domain` | `bi_server1.log` | WebLogic 관리 서버 오류 |
| `admin` | `AdminServer.log` | WebLogic Admin 서버 오류 |
| `ohs` | `error_log` | OHS 접근 오류, 프록시 오류 |
...
계정이 삭제된 사용자가 소유한 고아(Orphan) 콘텐츠를 탐지합니다.

**사용 순서:**
1. **스캔 실행** 버튼 클릭
2. 카탈로그 전체 항목과 현재 사용자 목록을 교차 분석
3. 결과 목록에서 정리 대상 콘텐츠 확인

**활용:** 퇴사자 계정 정리 후 고아 콘텐츠 정리. 스토리지 확보 및 카탈로그 유지보수.

---

### 🔼 업그레이드 진단

OAS/OBIEE → OAS 최신 버전 업그레이드 전 사전 진단을 수행합니다.

**사용 순서:**
1. 현재 버전 정보 입력 (자동 감지 또는 수동 입력)
2. 목표 버전 선택
3. **진단 실행** 클릭
4. 체크리스트 결과 확인 및 보고서 다운로드

| 진단 항목 | 내용 |
|---|---|
| 버전 호환성 | 현재 → 목표 버전 지원 여부 |
| 커스터마이징 | 업그레이드 영향 받는 사용자 정의 항목 |
| 사전 조치 | 업그레이드 전 필수 수행 작업 |
