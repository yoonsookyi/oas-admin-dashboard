...
document_root_from_conf() {
  ...
  local raw conf_dir instance_dir domain_home component_type component_name candidate
  raw=$(awk 'tolower($1)=="documentroot" {print $2; exit}' "$conf" 2>/dev/null | tr -d '"')
  conf_dir=$(cd "$(dirname "$conf")" && pwd)

  instance_dir="$conf_dir"
  [[ "$(basename "$instance_dir")" == "conf" ]] && instance_dir=$(dirname "$instance_dir")

  domain_home="${OHS_DOMAIN_HOME:-}"
  if [[ -z "$domain_home" && "$conf" == */config/fmwconfig/components/* ]]; then
    domain_home=$(echo "$conf" | sed 's|/config/fmwconfig/components/.*||')
  fi

  component_type=$(echo "$conf" | sed -n 's|.*/components/\([^/]*\)/instances/.*|\1|p')
  component_name=$(echo "$conf" | sed -n 's|.*/instances/\([^/]*\).*|\1|p')
  [[ -z "$component_type" ]] && component_type="OHS"
  [[ -z "$component_name" ]] && component_name=$(basename "$instance_dir")

  if [[ -n "$raw" ]]; then
    candidate="$raw"
    if [[ -n "$domain_home" ]]; then
      candidate=${candidate//'${ORACLE_INSTANCE}'/$domain_home}
      candidate=${candidate//'$ORACLE_INSTANCE'/$domain_home}
      candidate=${candidate//'${DOMAIN_HOME}'/$domain_home}
      candidate=${candidate//'$DOMAIN_HOME'/$domain_home}
      candidate=${candidate//'${OHS_DOMAIN_HOME}'/$domain_home}
      candidate=${candidate//'$OHS_DOMAIN_HOME'/$domain_home}
    fi
    candidate=${candidate//'${COMPONENT_TYPE}'/$component_type}
    candidate=${candidate//'$COMPONENT_TYPE'/$component_type}
    candidate=${candidate//'${COMPONENT_NAME}'/$component_name}
    candidate=${candidate//'$COMPONENT_NAME'/$component_name}
  ...
  # Common OHS layouts:
  #   .../instances/ohs1/httpd.conf        -> .../instances/ohs1/htdocs
  #   .../instances/ohs1/conf/httpd.conf   -> .../instances/ohs1/htdocs
  [[ -d "$instance_dir/htdocs" ]] && cd "$instance_dir/htdocs" && pwd && return
  [[ -d "$conf_dir/htdocs" ]] && cd "$conf_dir/htdocs" && pwd && return
}
...
printf "  |  %-20s %s\n" "OHS httpd.conf"  "${OHS_CONF:-not set}"
if [[ "$DEPLOY_OPT" == "b" ]]; then
